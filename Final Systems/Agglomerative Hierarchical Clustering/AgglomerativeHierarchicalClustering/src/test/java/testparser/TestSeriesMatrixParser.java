...
public class TestSeriesMatrixParser {
  ...
  public void setUp() throws Exception {
    ...
    emptyPdf = new File(baseDirectoryPath + File.separator + "emptypdf.pdf");
    emptyTxt = new File(baseDirectoryPath + File.separator + "emptyfile.txt");
  ...
  /**
   * Test to try and get a formatted name from the Series Matrix Parser.
   * This should return 'Gene Micro-Array Series Matrix Parser'.
   */
  @Test
  public void testGetFormattedName() {
    assertEquals("Could not get a formatted name",
        "Gene Micro-Array Series Matrix Parser", testParser.getName());
  }
  
  /**
   * Test to try and get a formatted description from the Series Matrix Parser.
   * This should return a formatted description.
   */
  @Test
  public void testGetFormattedDescription() {
    assertEquals("Could not get a formatted description",
        "Parses Gene Micro-Array Series Matrix Files", testParser.getDescription());
  }
  
  /**
   * Test to try and see how the parser handles a pdf file. 
   * This should not throw an error with the message being 'the file type is not supported'.
   * @throws IOException - Thrown if the file cannot be read.
   * @throws FileNotFoundException - Thrown if the file cannot be found.
   */
  @Test
  public void testParserHandlePdf() throws FileNotFoundException, IOException {
    try {
      testParser.parseFile(emptyPdf);
    } catch (UnsupportedOperationException ex) {
      assertEquals("Could not get the expected error message for a unsupported file",
          "This file type is not supported with this parser", ex.getMessage());
    }
  }
  
  /**
   * Test to try and get the supported file extensions. This should be 2 items, long with the 
   * first being the description: txt files (*.txt), with the second being *.txt.
   */
  @Test
  public void testGetSupportedFileTypes() {
    ArrayList<String> returnedTypes = testParser.getSupportedFileExtensions();
    assertEquals("Could not get the expected description",
        "txt files (*.txt)", returnedTypes.get(0));
    assertEquals("Could not get the correct supported types",
        "*.txt", returnedTypes.get(1));
  }
  
  /**
   * Test to see if the parser can handle a file that does not have the series matrix 
   * begin line in it.
   * This should throw an unsupported exception as with an incorrect file type.
   * This should also have the same error message.
   * @throws IOException - Thrown if the file cannot be read.
   * @throws FileNotFoundException - Thrown if the file cannot be found.
   */
  @Test
  public void testParserHandleInvalidTxt() throws FileNotFoundException, IOException {
    try {
      testParser.parseFile(emptyTxt);
    } catch (UnsupportedOperationException ex) {
      assertEquals("Could not get the expected error message for a unsupported file",
          "This file type is not supported with this parser", ex.getMessage());
    }
